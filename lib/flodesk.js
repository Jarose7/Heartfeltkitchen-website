...
async function ensureSegmentId() {
  ...
  // Flodesk requires a valid color when creating a segment (confirmed
  // 2026-09-01 in production: omitting it returns a 400 "Invalid color"
  // even though the API docs don't mark it as required) — so fetch the
  // list of colors Flodesk actually accepts and use the first one, rather
  // than guessing a hex/name that might not be on their allowed list.
  const colors = await flodeskRequest(`/segments/colors`);
  const color = Array.isArray(colors) && colors.length ? colors[0] : undefined;

  const created = await flodeskRequest(`/segments`, {
    method: "POST",
    body: JSON.stringify({ name: SEGMENT_NAME, color }),
  });
...
async function sendInquiryToFlodesk(inquiry) {
  if (!process.env.FLODESK_API_KEY) {
    // Logged (not silent) so "nothing shows up in Flodesk" is easy to tell
    // apart from "it tried and failed" vs "it never ran at all" — this
    // line should be visible on every server boot below, separately, so
    // you can confirm the key is actually being read before an inquiry
    // even comes in.
    console.log("[flodesk] FLODESK_API_KEY not set — skipping Flodesk sync for this inquiry.");
    return;
  }
  if (!inquiry || !inquiry.email) {
    console.log("[flodesk] Skipping sync — inquiry missing an email.");
    return;
  }

  console.log(`[flodesk] Syncing inquiry to Flodesk for ${inquiry.email}...`);
  ...
  try {
    ...
    const subscriber = await flodeskRequest(`/subscribers`, {
      method: "POST",
      body: JSON.stringify({
        email: inquiry.email,
    ...
    console.log(`[flodesk] Synced OK — subscriber id ${subscriber && subscriber.id} (${inquiry.email}), segment ${segmentId}.`);
  } catch (err) {
    console.error("[flodesk] Failed to sync inquiry (non-fatal, inquiry is still saved in our DB):", err.message);
  }
}

// Boot-time visibility: log once at startup whether the integration is
// configured at all, so this can be confirmed from the deploy logs alone,
// without needing to submit a test inquiry first.
console.log(
  "[flodesk] Integration",
  process.env.FLODESK_API_KEY ? "configured (FLODESK_API_KEY is set)." : "NOT configured (FLODESK_API_KEY is unset — inquiries will only save to the database)."
);
